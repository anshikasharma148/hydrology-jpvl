...
router.post("/login", async (req, res) => {
  try {
    ...
    const [user] = await cdcDB.query("SELECT * FROM users WHERE email = ?", [email]);
...
router.post("/update-password", async (req, res) => {
  try {
    ...
    const [userRows] = await cdcDB.query("SELECT * FROM users WHERE email = ?", [email]);
...
router.delete("/delete/:id", authenticate, isAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    await cdcDB.query("DELETE FROM users WHERE id = ?", [id]);
